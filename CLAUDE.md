...
- Healing, an unanswered (non-duel) attack, AND a **tentacle duel** (two opposing tentacles between the same two cells) are all capped at the source cell's production budget (`_flowBudget`/`_boostShare`, shared across a cell's active outgoing + clashing tentacles) — a cell's stored reserve never drains just from fighting. `battleFeed` (used only by duels) draws from the exact same budget as one-sided flow, so two equally-produced cells dueling each other feed identical amounts and settle into a genuine stalemate (modulo the small home-field nudge below) instead of racing each other's reserves to zero. The only way to *win* a duel is superior supply: higher own production, or — via the overflow/symbiosis forwarding below — being topped up by another of your own cells, which raises `_boostShare` and lets that side feed more than production alone would allow, slowly grinding the front forward. In the clash resolution the loser always yields ground (decoupled from whether the winner can afford to advance — otherwise two depleted cells froze the front forever and an attacked 0-unit cell was never captured), and the home-field bias (`clashHomefield`) only applies to a cell whose stored reserve exceeds `CONFIG.clashHoldMin` — an **empty cell cannot hold a front**, so a supplied attacker breaks through. When *both* clashing cells are exhausted (reserve ≤ `clashHoldMin`), `net` is only production noise (~0) and neither home-field applies, which previously left the front frozen at two 0-unit cells forever — so a tie-breaker kicks in: the tentacle with the shorter remaining distance to its target (`len - head`) gets a small, self-reinforcing advance (`CONFIG.clashBreak` px/s) that always resolves the duel into a capture. It fires *only* when both are empty, so real supply advantages are untouched.
